...
def main(argv):
    ...
    parser.add_argument('-m', '--model', action='append', nargs=2, required=True, help='[MODEL_DIR] [LABELS_FILE.pbtxt]: Path to a model\'s directory and path to its labels .pbtxt file')
    parser.add_argument('-p', '--port', default=default_port, help='Server\'s port number, default: ' + default_port)
    parser.add_argument('-d', '--no-debug', action='store_true', help='Disable writing debug images.')
    parser.add_argument('-n', '--name', default='fetch-server', help='Service name')
